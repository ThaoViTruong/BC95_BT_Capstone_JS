(() => {
  ...
  const createPageButton = ({ label, page, active, disabled }) => {
    ...
    btn.type = "button";
    btn.className =
      "inline-flex h-10 min-w-10 items-center justify-center rounded-full border px-3 text-sm font-semibold shadow-sm outline-none transition focus:ring-2 focus:ring-teal-200";
  ...
  const createProductCard = (p, utils) => {
    ...
    card.className =
      "overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-md";

    const imageWrap = document.createElement("div");
    imageWrap.className =
      "h-80 w-full bg-gradient-to-br from-slate-900 to-slate-700 md:h-96";
    ...
    btn.type = "button";
    btn.className =
      "mt-4 inline-flex w-full items-center justify-center gap-2 rounded-full bg-slate-900 px-4 py-2 text-sm font-semibold text-white transition hover:bg-slate-800 active:scale-[0.99]";
    btn.innerHTML =
  ...
  const renderCurrentPage = (els, utils) => {
    ...
    if (!currentProducts.length) {
      ...
      empty.className =
        "col-span-full rounded-2xl border border-dashed border-slate-300 bg-white p-10 text-center text-slate-500";
  ...
  const applyFilterAndSort = (els, state, utils) => {
    ...
    const result = selectedType
      ? source.filter((p) => utils.normalizeType(p.type) === selectedType)
      : [...source];
  ...
  const bindProductEvents = (els, state, utils) => {
    if (els?.typeSelect) {
      els.typeSelect.onchange = () => handleTypeChange(els, state, utils);
    }
    if (els?.sortSelect) {
      els.sortSelect.onchange = () => handleSortChange(els, state, utils);
    }
    els?.productGrid?.addEventListener("click", (e) =>
      handleProductGridClick(e, state, els, utils)
    );
    els?.pagination?.addEventListener("click", (e) =>
      handlePaginationClick(e, els, utils)
    );
  };

  window.shoppingProductsController = window.shoppingProductsController || {};
  window.shoppingProductsController.renderProducts = renderProducts;
  window.shoppingProductsController.bindProductEvents = bindProductEvents;
})();
